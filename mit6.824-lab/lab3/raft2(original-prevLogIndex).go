...
func (rf *Raft) checkElectionLoop() {
	for !rf.killed() {
		...
		fmt.Printf("主机:%d,term:%d,role:%v,len(logs):%v,lastLogicIndex:%v,HB为false,变为candidate\n",rf.me,rf.currentTerm,rf.role,len(rf.logs),rf.getLastLogicIndex())
		rf.mu.Lock()
		rf.turnCandidate()
		rf.persist()
		rf.mu.Unlock()
...
func (rf *Raft) applyLoop() {
	for !rf.killed() {
		...
		for rf.lastApplied < rf.commitIndex {
			...
			fmt.Printf("主机:%d, lastApplied:%d, commitIndex:%d,len(logs):%v,lastIncludedIndex:%v\n",rf.me,rf.lastApplied,rf.commitIndex,len(rf.logs),rf.lastIncludedIndex)
			rf.apply(rf.lastApplied, rf.logs[rf.subLastIncludedIndex(rf.lastApplied)], rf.applyCh)
		}
		rf.mu.Unlock()
...
func (rf *Raft) turnLeader() {
	...
	for i:=0;i<len(rf.peers);i++ {
		...
		rf.matchIndex[i] = rf.addLastIncludedIndex(0)
		rf.nextIndex[i] = rf.addLastIncludedIndex(len(rf.logs))
	}
	go rf.pingLoop()
...
func (rf *Raft) persist() {
	...
	err4 := e.Encode(rf.lastIncludedIndex)
	if err4 != nil {
		fmt.Printf("主机%v,term:%v,encode lastIncludedIndex error:%v\n",rf.me,rf.currentTerm,err4)
	}
	err5 := e.Encode(rf.lastIncludedTerm)
	if err5 != nil {
		fmt.Printf("主机%v,term:%v,encode lastIncludedTerm error:%v\n",rf.me,rf.currentTerm,err5)
	}
	/*err6 := e.Encode(rf.lastApplied)
	if err6 != nil {
		fmt.Printf("主机%v,term:%v,encode lastApplied error:%v\n",rf.me,rf.currentTerm,err6)
	}
	err7 := e.Encode(rf.commitIndex)
	if err7 != nil {
		fmt.Printf("主机%v,term:%v,encode commitIndex error:%v\n",rf.me,rf.currentTerm,err7)
	}*/
	data := w.Bytes()
	rf.persister.SaveRaftState(data)
}
...
func (rf *Raft) readPersist(data []byte) {
	...
	var voteFor1 int
	var logs1 []Log
	var lastIncludedIndex1 int
	var lastIncludedTerm1 int
	...
	err4 := d.Decode(&lastIncludedIndex1)
	if err4 != nil {
		fmt.Printf("readPersist,decode lastIncludedIndex err:%v\n",err4)
	}
	err5 := d.Decode(&lastIncludedTerm1)
	if err5 != nil {
		fmt.Printf("readPersist,decode lastIncludedTerm err:%v\n",err5)
	}
	if err1==nil && err2==nil && err3==nil && err4==nil && err5==nil {
		rf.currentTerm = currentTerm1
		rf.votedFor = voteFor1
		rf.logs = logs1
		rf.lastIncludedIndex = lastIncludedIndex1
		rf.lastIncludedTerm = lastIncludedTerm1
	}
}
...
//
// send an InstallSnapshot RPC to a follower when
// the leader has discarded log entries that the follower needs.
// When a follower receives an InstallSnapshot RPC,
// it must hand the included snapshot to its kvserver.
// You can use the applyCh for this purpose, by adding new fields to ApplyMsg.
//
...
func (rf *Raft) RequestVote(args *RequestVoteArgs, reply *RequestVoteReply) {
	...
	if args.TERM < rf.currentTerm { // 接收到的term比自己的term小
		...
		} else if args.LASTLOGTERM == rf.logs[len(rf.logs)-1].TERM && (args.LASTLOGINDEX < rf.getLastLogicIndex()) {
...
func (rf *Raft) sendRequestVote(server int, term int) {
	...
	for i:=0; i< len(rf.peers); i++ {
		...
		go func(x int) bool {
			...
			args.LASTLOGINDEX = rf.getLastLogicIndex()
	...
	if (*args).TERM < rf.currentTerm {
		...
		if args.PREVLOGINDEX == -1 {
			...
			if args.PREVLOGINDEX >= len(rf.logs) || rf.logs[args.PREVLOGINDEX].TERM != args.PREVLOGTERM {
				//fmt.Printf("主机:%v, 日志不匹配,term:%v,len(logs):%v\n",rf.me, rf.currentTerm,len(rf.logs))
				...
				(*reply).TERM = rf.currentTerm
				(*reply).SUCCESS = false
				//fmt.Printf("681,主机%v,len(logs):%v,commitIndex:%v,lastIIdx:%v,args.prevIdx:%v,leaderCmtIdx:%v,len(args.entries):%v\n",rf.me,len(rf.logs),rf.commitIndex,rf.lastIncludedIndex,args.PREVLOGINDEX,args.LEADERCOMMIT,len(args.ENTRIES))
				return
			}
		...
		if args.LEADERCOMMIT > rf.commitIndex {
			...
			rf.commitIndex = minInt(args.LEADERCOMMIT, rf.addLastIncludedIndex(len(rf.logs)-1))
		}

		//fmt.Printf("主机:%v, commitIndex:%v, term:%v\n",rf.me, rf.commitIndex, rf.currentTerm)
		(*reply).TERM = rf.currentTerm
		(*reply).SUCCESS = true
		//fmt.Printf("698,主机%v,len(logs):%v,commitIndex:%v,lastIIdx:%v,args.prevIdx:%v,leaderCmtIdx:%v,len(args.entries):%v,lastIncludedIndex:%v\n",rf.me,len(rf.logs),rf.commitIndex,rf.lastIncludedIndex,args.PREVLOGINDEX,args.LEADERCOMMIT,len(args.ENTRIES),rf.lastIncludedIndex)
		return
	}
}
...
func (rf *Raft) SendAppendEntries(term int) {
	...
	for i:=0; i< len(rf.peers); i++ {
		if i==rf.me {
			...
			rf.nextIndex[rf.me] = rf.addLastIncludedIndex(len(rf.logs))
			rf.matchIndex[rf.me] = rf.addLastIncludedIndex(len(rf.logs)-1)
			continue
		}

		nextEntryIndex := rf.nextIndex[i]
		if nextEntryIndex <= rf.lastIncludedIndex {
			rf.SendInstallSnapshot(i, term)
			continue
		}

		//wg.Add(1)
		go func(x, y int) bool {   // x, y对应i, term
			if rf.role != LEADER {
				return false
			}
			...
			rf.mu.Lock()
			prevLogIndex := rf.subLastIncludedIndex(rf.nextIndex[x]) - 1
			args.TERM = y
			args.LEADERID = rf.me
			...
			if prevLogIndex >= 0 {
				...
				args.ENTRIES = rf.logs[rf.subLastIncludedIndex(rf.nextIndex[x]):]
			}else {
				...
				if (&reply).TERM > rf.currentTerm {
					...
					if rf.role == LEADER && rf.currentTerm == args.TERM {
						...
						if (&reply).SUCCESS {
							...
							rf.matchIndex[x] = rf.addLastIncludedIndex(args.PREVLOGINDEX + len(args.ENTRIES))
							...
							if rf.subLastIncludedIndex(majorAgreementIndex) >= 0 && len(rf.logs) > rf.subLastIncludedIndex(majorAgreementIndex) && rf.logs[rf.subLastIncludedIndex(majorAgreementIndex)].TERM == rf.currentTerm && majorAgreementIndex > rf.commitIndex {
								rf.commitIndex = majorAgreementIndex
								//fmt.Printf("主机:%d, 当前term:%d, 更新commitIndex为:%d\n",rf.me,rf.currentTerm,rf.commitIndex)
							}
...
func (rf *Raft) Start(command interface{}) (int, int, bool) {
	...
	if rf.role == LEADER {
		...
		index = rf.addLastIncludedIndex(len(rf.logs)-1)
		term = rf.currentTerm
		isLeader = true
		fmt.Printf("leader:%d,term:%v new log:%v, len(logs):%v, LastLogicIndex:%v,lastIncludedIndex:%v,lastApplied:%v,commitIndex:%v\n",rf.me, rf.currentTerm, log.COMMAND, len(rf.logs),rf.getLastLogicIndex(),rf.lastIncludedIndex,rf.lastApplied,rf.commitIndex)
		rf.mu.Unlock()
		rf.SendAppendEntries(rf.currentTerm)
...
func Make(peers []*labrpc.ClientEnd, me int,
	...
	rf.lastApplied = rf.addLastIncludedIndex(rf.lastApplied)
	rf.commitIndex = rf.addLastIncludedIndex(rf.commitIndex)
	rf.mu.Unlock()
...
// logicIndex表示逻辑日志位置, 下面几个函数计算偏移量, 即在数组中的实际位置

// logicIndex>rf.lastIncludedIndex才能使用
func (rf *Raft) getLog(logicIndex int) Log {
	return rf.logs[logicIndex - rf.lastIncludedIndex]
}
//
func (rf *Raft) getLastLogicIndex() int {
...
// 快照中的lastIncludedIndex和lastIncludedTerm对应日志项不包含在日志数组中
func (rf *Raft) ProduceSnapshot(commandIndex int, snapshot []byte)  {
	rf.mu.Lock()
	defer rf.mu.Unlock()

	if commandIndex <= rf.lastIncludedIndex { // kvServer调用produceSnapshot()需要竞争锁，可能新快照先拿到锁，旧的commandIndex对应log已经被删除
		return
	}

	// 抛弃之前及包括commandIndex的log
	log := Log{}
	var command interface{} = 1
	log.COMMAND = command
	log.TERM = -1

	rf.lastIncludedTerm = rf.logs[rf.subLastIncludedIndex(commandIndex)].TERM
	logs1 := rf.logs[(commandIndex-rf.lastIncludedIndex+1):] // 注意，这里可能越界
	rf.logs = []Log{log}
	// 丢弃commandIndex以前的日志
	rf.logs = append(rf.logs, logs1...)
	fmt.Printf("主机%v, commandIndex:%v, len(rf.logs):%v, lastIncludedIndex:%v, 更新后的lastIncludedTerm:%v, curTerm:%v\n",rf.me,commandIndex,len(rf.logs),rf.lastIncludedIndex,rf.lastIncludedTerm,rf.currentTerm)
	rf.lastIncludedIndex = commandIndex
	//rf.lastIncludedTerm = rf.logs[1].TERM  //越界，原因未知
	rf.persistStateSnapshot(snapshot)
}
	...
	go func(x, y int) {
		...
		if ok {
			if (&reply).Term > rf.currentTerm {
				...
				if rf.role == LEADER && rf.currentTerm == args.Term {
					rf.matchIndex[x] = args.LastIncludedIndex
					rf.nextIndex[x] = args.LastIncludedIndex + 1
				}
			}
		}
	}(server, term)
...
func (rf *Raft) InstallSnapshot(args *InstallSnapshotArgs, reply *InstallSnapshotReply) {
	...
	if args.Term < rf.currentTerm {
		...
		rf.mu.Lock()
		if args.LastIncludedIndex < rf.getLastLogicIndex() {
			if rf.logs[rf.subLastIncludedIndex(args.LastIncludedIndex)].TERM == args.LastIncludedTerm {
				logs1 := rf.logs[(args.LastIncludedIndex-rf.lastIncludedIndex+1):]
				log := Log{}
				var command interface{} = 1
				log.COMMAND = command
				log.TERM = -1
