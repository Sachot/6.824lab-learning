...
2.这个实验由于需要大量改动lab2代码，多处index需要加减快照位移量，很容易出错，到目前为止我所实现的lab3B代码并不能完全通过100次测试；

出现的问题：
TestSnapshotSize3B非常慢，测试用例无法通过，超时约50%。
...
// configNum-(shard-(key,value)), 存有不再负责的分片数据，例如
// config2不再负责的分片4数据存储在outShardKv[1][4]中
missingShardsKv map[int]int 
// 存储当前缺少的shard，当missingShardKv不为空，表示当前config           
...
第二种是整个group与master断连。这会导致原来shard-bucket中的数据丢失，lab中并没有实现对这种情况的容错机制，容错基于单个server发生故障。但是在真正生产环境中，可能会因为自然灾害、数据中心断电等导致整个group断开连接。
